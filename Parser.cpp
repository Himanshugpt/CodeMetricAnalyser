...
int main(int argc, char* argv[])
{
  ...
  for(int i=1; i<argc; ++i)
  {
	...
	ConfigParserForFunction configFunction;
	ControlSpanParser controlSpanParser;
   // Parser* pParser = configure.Build();
	Parser* pParser1 = configFunction.Build();
	Parser* pControlSpanParser = controlSpanParser.Build();
    ...
    try
    {
      ...
      {
		  //std::cout << "\npParser1 raedy..";
        if(!configFunction.Attach(argv[i]))
        {
    ...
    catch(std::exception& ex)
    {
	 ...
	 try
    {
      if(pControlSpanParser)
      {
		  std::cout << "\npParser1 raedy..";
        if(!controlSpanParser.Attach(argv[i]))
        {
          std::cout << "\n  could not open file " << argv[i] << std::endl;
          continue;
        }
      ...
      // now that parser is built, use it
	  while(pControlSpanParser->next())
        pControlSpanParser->parse();
        std::cout << "\n\n";
		//TypeTable::Contains("sd");
    }
    catch(std::exception& exq)
    {
		std::cout << "ecxeption :: ";
      std::cout << "\n\n    " << exq.what() << "\n\n";
    }
